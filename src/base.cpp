...
userver::formats::json::Value Base::CreateObject(
    ...
    // from
    // https://userver.tech/da/dd0/classstorages_1_1postgres_1_1ParameterStore.html
    ...
    if (res.RowsAffected()) {
        ...
        return utils::ResponseMessage(
            fmt::format("{} was created!", target_type));
    }
...
userver::formats::json::Value Base::UpdateObject(
    ...
    if (res.RowsAffected()) {
        ...
        return utils::ResponseMessage(
            fmt::format("{} was updated!", target_type));
    }
...
userver::formats::json::Value Base::DeleteObject(
    const userver::server::http::HttpRequest& request,
    const std::string_view target_type, const std::string_view target_table,
    ...
    auto transaction = pg_cluster_->Begin(
        ...
        userver::storages::postgres::ClusterHostType::kMaster, {});
    auto project =
        transaction.Execute(fmt::format("SELECT * FROM {} WHERE {}_id = $1",
    ...
    auto res = transaction.Execute(
        fmt::format("DELETE FROM {} WHERE {}_id = $1 AND user_id = $2",
                    target_table, target_type),
        std::stoi(object_id.data()), std::stoi(user_id));

    if (res.RowsAffected()) {
        transaction.Commit();
        request.SetResponseStatus(userver::server::http::HttpStatus::kOk);
        return utils::ResponseMessage(
            fmt::format("{} was deleted!", target_type));
    }
...
userver::formats::json::Value Base::GetComments(
    ...
    auto iter_result_set = result_set.AsSetOf<utils::CommentTuple>(
...
std::string Base::CheckAndHash(
    ...
    userver::storages::postgres::ResultSet candidate =
        transaction.Execute("SELECT * from users WHERE username = $1",
    ...
    if (!body["password"].As<std::string>().empty()) {
        ...
        v["password"] =
            userver::crypto::hash::Sha256(body["password"].As<std::string>());
        body = v.ExtractValue();
    }
