...
namespace portfolio::utils {
  userver::formats::json::Value CreateJsonResult(
    ...
    for (auto row : iteration) {
      ...
      auto [user_id, name, last_name, username, email, phone_number,
            password, profession, key_skills, about, country, city, user_avatar,
            banner, created_at, updated_at] = row;
  ...
  userver::formats::json::Value CreateJsonResult(
    ...
    for (auto row : iteration) {
      ...
      auto [post_id, user_id, name, text, media, tags,
