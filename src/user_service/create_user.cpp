    ...
    public:
      ...
      std::string HandleRequestThrow(
        ...
        for (auto [key, value] : userver::formats::json::Items(body)) {
          error = portfolio::user::ValidateRegistration(key, value.ConvertTo<std::string>());
          if (!error.empty()) {
        ...
        std::string hash_password = userver::crypto::hash::Sha256(password);
        ...
        if (res.RowsAffected()) {
          ...
          return fmt::format("User {} was created!\n", username);
        }
